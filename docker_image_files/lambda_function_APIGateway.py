...
def predict(image_url): # image_url <class '_io.BytesIO'>
    ...
    model = mlflow.onnx.load_model(model_uri=model_uri, dst_path=LAMBDA_TMP)
    start_time = time.monotonic()
    model = mlflow.onnx.load_model(model_uri=model_uri, dst_path=LAMBDA_TMP) # returns <class 'onnx.onnx_ml_pb2.ModelProto'>
    ...
    print(f'serialized model in {(serialized_time-downloaded_time):.2f}s')
